<template>
    <dashboard-modal :modal-id="userAndRoles.users.manageUserModalId"
                     :title="modalTitle"
                     :preloader="preloader"
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            $('#manage-users-modal').on('shown.bs.modal', function (e) {
                instance.modalHeight =  $("#manage-users-modal .modal-dialog").height();

                if (instance.modalHeight > 530) {
                    instance.modalScroll = true
                } else {
                    instance.modalScroll = false
                }
            });
        },
        watch: {
